H5P.ReactionTimeGame.TimerWidget = (function ($) {

  /**
   * Creates a TimerWidget instance
   *
   * @class
   * @namespace H5P.ReactionTimeGame
   *
  ...
  function TimerWidget(t) {
    ...
    var $timer = $('<time>', {
      ...
      role: 'timer',
      'aria-hidden': true
      // text: H5P.ReactionTimeGame.tReplace(t.time, {time: '00:00'})
    });
    ...
    var $ariaTimer = $('<time>', {
      ...
      role: 'timer',
      tabindex: '-1'
      // text: H5P.ReactionTimeGame.tReplace(t.time, {time: '0'})
    });
     ...
     */
    var update = function () {
      // $timer.text(H5P.ReactionTimeGame.tReplace(t.time, {time: humanizeTime(getTime()/1000)}));
      //
      // if (incrementingAria) {
      //   $ariaTimer.text(H5P.ReactionTimeGame.tReplace(t.time, {time: humanizeTime(getTime()/1000, ', ')}));
      // }
